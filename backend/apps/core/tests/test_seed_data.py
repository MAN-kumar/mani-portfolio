...
class SeedDataCommandTests(TestCase):
    def test_seed_data_first_run_creates_records(self):
        ...
        output = out.getvalue()
        self.assertIn("Successfully seeded all canonical portfolio data", output)

        # Verify Profile and SocialLinks
        self.assertEqual(Profile.objects.count(), 1)
        profile = Profile.objects.first()
        self.assertEqual(profile.name, "Mani")
        ...
        self.assertTrue(Project.objects.filter(slug="interactive-portfolio-platform").exists())

        p1 = Project.objects.get(slug="interactive-portfolio-platform")
        ...
        r1 = Research.objects.get(slug="explainable-ai-dashboard")
        self.assertEqual(r1.related_projects.count(), 2)
        ...
        self.assertEqual(ExperimentResult.objects.filter(experiment=exp).count(), 2)
    ...
    def test_seed_data_second_run_is_idempotent(self):
        ...
        self.assertIn("Successfully seeded all canonical portfolio data", output2)
        ...
        self.assertEqual(ExperimentResult.objects.count(), 2)
        self.assertEqual(Skill.objects.count(), 4)
        self.assertEqual(Experience.objects.count(), 1)
        self.assertEqual(Education.objects.count(), 1)
        self.assertEqual(Achievement.objects.count(), 1)
